...
export const HandCursor: React.FC<HandCursorProps> = ({
  ...
  // Directly update position at screen refresh rate for ultra-smooth tracking
  useEffect(() => {
    const updatePos = () => {
      const el = cursorRef.current;
      if (el && trackingFrameRef.current) {
        const frame = trackingFrameRef.current;
        el.style.transform = `translate(${frame.screenPos.x}px, ${frame.screenPos.y}px) translate(-50%, -50%)`;
        el.style.opacity = frame.handDetected ? '1' : '0';
      }
  ...
  const frame = trackingFrameRef.current;
  const palmOpen = frame ? frame.palmOpen : true;
  ...
  return (
    <div
      ref={cursorRef}
      className="fixed top-0 left-0 pointer-events-none z-50 transition-opacity duration-150 will-change-transform"
      style={{
        transform: `translate(${frame.screenPos.x}px, ${frame.screenPos.y}px) translate(-50%, -50%)`
      }}
    >
      {isGameplayMode ? (
        ...
        <div className="relative flex items-center justify-center">
          ...
          <div
            className={`rounded-full border-2 border-dashed transition-all duration-150 ${
              palmOpen
                ? 'border-emerald-400 bg-emerald-500/20 scale-100 shadow-[0_0_30px_rgba(52,211,153,0.7)]'
          ...
          <div className="absolute flex flex-col items-center justify-center">
            <svg
              className={`w-16 h-16 drop-shadow-[0_4px_12px_rgba(0,0,0,0.9)] transition-transform duration-150 ${
            ...
            <span
              className={`mt-1 text-[11px] font-black uppercase px-2.5 py-0.5 rounded-full backdrop-blur-md shadow-md ${
                palmOpen
                  ? 'bg-emerald-500 text-slate-950'
                  : 'bg-amber-500 text-slate-950'
              }`}
            >
              {palmOpen ? 'CATCH ZONE' : 'OPEN PALM'}
            </span>
          </div>
        </div>
      ) : (
